...
public class UserLogMQServiceImpl implements UserLogMQService {
    ...
    @Override
    public void createUserLog(long uid, byte type, String before, String after, String ip, String ua) {

        UserLogTask task = UserLogTask.builder().uid(uid).type(type).before(before).after(after).ip(ip).ua(ua).build();
    ...
    private void executeTask(UserLogTask task) {
        ...
        try {
            ...
            userLogService.createUserLog(task.getUid(), task.getType(), task.getBefore(), task.getAfter(), task.getIp(), task.getUa());
        } catch (Exception e) {
